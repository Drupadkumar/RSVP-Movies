...
-- FINDING THE COUNT OF NULL VALUES IN EVERY COLUMN OF MOVIE TABLE USING CASE WHEN STATEMENT
...
-- CREATING COMMON TABLE EXPRESSION AND THEN USING IT IN THE MAIN QUERY 
...
-- TO FIND THE THRILLER GENRE RANK USING CTE CREATED ABOVE 
SELECT 
    *
FROM
    rank_of_thriller
...
(
    SELECT 
        ...
        COUNT(di.movie_id) AS movie_count, 
        ROW_NUMBER() OVER (ORDER BY COUNT(di.movie_id) DESC) AS director_rank
    FROM director_mapping di 
        INNER JOIN names n ON n.id = di.name_id 
        INNER JOIN genre g ON di.movie_id = g.movie_id 
        INNER JOIN ratings r ON r.movie_id = di.movie_id
    WHERE 
        r.avg_rating > 8 
            AND 
...
top_5 AS (
    SELECT 
        ...
        CAST(REPLACE(REPLACE(IFNULL(m.worldwide_gross_income, '0'), 'INR', ''), '$', '') AS DECIMAL(10)) AS worldwide_gross_income,
        DENSE_RANK() OVER (PARTITION BY g.genre, m.year ORDER BY CAST(REPLACE(REPLACE(IFNULL(m.worldwide_gross_income, '0'), 'INR', ''), '$', '') AS DECIMAL(10)) DESC) AS movie_rank
    FROM 
        movie AS m 
            INNER JOIN 
...
avg_inter_days AS
(
   SELECT name_id, AVG(diff) AS avg_movie_days
...
(
   SELECT d.name_id AS director_id,
    ...
    ROUND(avg_movie_days) AS avg_inter_movie_days,
